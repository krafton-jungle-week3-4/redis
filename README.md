# Mini Redis
<img width="2386" height="1412" alt="image" src="https://github.com/user-attachments/assets/e5e94f89-00e5-4e06-9425-52cb0ea95e62" />
...
또한 Git Action을 이용해서 하나의 사이클이 끝날 때마다 Testcode를 실행시켜 진행도를 확인할 수 있게 했습니다.

## 10. 프로젝트를 통해 배운 점

Codex를 활용하면서, 요청을 한 번에 길게 맡기기보다 작업을 작은 단위로 나누고 병렬적으로 운영하는 방식의 중요성을 깨달았습니다.
특히 테스트 코드 작성을 요청한 뒤 결과를 기다리는 시간이 길어졌는데, 이 과정에서 작업을 세분화해 여러 흐름으로 동시에 진행했다면 전체 시간을 줄이고 효율을 더 높일 수 있었겠다는 인사이트를 얻었습니다.
앞으로는 기능 구현, 테스트 작성, 검증 과정을 병렬적으로 설계하여 Codex 활용 효율을 극대화하고자 합니다.

또한 개발에 앞서 API 명세를 명확히 정의한 것이 프로젝트 전반에 큰 도움이 되었습니다.
각 기능의 책임 범위가 분명해지면서 팀원 간 역할 분담이 자연스럽게 이루어졌고, 구현 과정에서의 해석 차이나 충돌도 크게 줄어들었습니다.
그 결과 브랜치 관리와 Git 병합 과정 역시 안정적으로 진행되어, 협업 생산성과 코드 품질을 동시에 향상시킬 수 있었습니다.

## 11. 역할 분담
이혜연, 유승이: Redis 서버 통합, Resp, 코어 로직, API 개발
임재환: Unit Test, 동시성 테스트, API 명세서 작성
홍윤기: Redis vs MongoDB 성능 비교
